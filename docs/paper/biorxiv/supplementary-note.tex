...
\date{}
\title{\textbf{Supplementary Note}\\Grounding biological data to databases}
...
Marker genes are often stored as cell type--gene pairs. That format is practical, but incomplete. In a single-cell experiment, a gene is called a marker because it distinguishes a target group from other groups in that experiment: sibling clusters, disease-specific subsets, a one-vs-rest background, or an atlas-level set of cell types. Changing those comparison backgrounds can change whether the gene is a marker. This note formalizes that point, and related ones, with Lean. We assume that marker calls are binary, as is common when genes are selected for follow-up experiments and in marker-panel methods for cell isolation \citep{Delaney2019COMET,Tangherloni2023MAGNETO}, and that markers are evaluated through pairwise comparisons between cell groups, a standard differential-expression workflow \citep{Robinson_2009}. The structure is simple: biological desiderata imply marker axioms, the axioms force a definition of marker genes and marker panels, practical desiderata define how useful marker panels are selected, and these objects motivate a marker-supported definition of cell type. Dropping the binary assumption changes how marker genes are scored, ranked, or thresholded, but it does not change the requirement that a marker specify the target and comparison set.

\section{Biological Desiderata and Marker Axioms}

Before defining a marker, we first state the biological desiderata that a marker rule should satisfy. A marker is not an absolute property of a gene. It is evidence that a target group can be distinguished from specified alternatives. The target group can be a cell type, a cell state, a tissue compartment, a disease stratum, or any other grouping used for marker analysis.
...
\toprule
Biological desideratum & Mathematical axiom \\
\midrule
A marker is meaningful only for a target group and the alternatives it must be distinguished from. & Marker validity is indexed by a target $t$ and comparison set $C$. \\
If there are no alternatives, there is no distinguishing task. & Empty-set base case: every gene is in the marker set for $C=\varnothing$, and every panel is valid for $C=\varnothing$. \\
For one alternative, the rule should agree with the ordinary pairwise marker call. & Pairwise agreement: for one comparison background $h$, the marker set is $M(t,h)$; a panel is valid if it contains at least one gene in $M(t,h)$. \\
To distinguish a target from many alternatives, the rule must work for each alternative. & Composition: validity over $C_1\cup C_2$ is validity over $C_1$ and over $C_2$. \\
\bottomrule
\end{tabular}
\caption{Biological desiderata and the corresponding marker axioms. These axioms define what it means for a marker gene or marker panel to be valid. They do not yet specify how to choose among many valid panels.}
\label{tab:marker-desiderata-axioms}
\end{table}

\begin{definition}[Marker gene axioms]\label{def:marker-family-axioms}
\normalfont
...
If $H$ is the set of all non-target groups in the partition, then every possible comparison set is an element of the power set $\mathcal{P}(H)$. The singleton sets $\{h\}$ are the smallest nonempty comparison sets and give ordinary pairwise comparisons. The full set $H$ is the largest comparison set and asks whether the target is distinguished from every observed alternative. These are the two set-theoretic ends of the lattice of comparison sets. A pooled one-vs-rest test also uses all groups in $H$, but it combines them into one background distribution; it is not the same as requiring a gene to pass every singleton comparison.
...
The no-rescue property is therefore a consequence, not an additional axiom. Adding required comparison backgrounds makes the marker rule stricter, not easier; it cannot rescue a gene that already failed one required pairwise contrast.

The same axioms also define marker panels. A marker gene must work in every pairwise contrast, so marker genes are intersections of pairwise marker sets. A marker panel is weaker and more practical: it only requires that each pairwise contrast be covered by at least one selected gene.

\begin{definition}[Marker panel]\label{def:marker-panel-hitting-set}
Let $P\subseteq G$ be a finite set of selected genes. The panel $P$ is valid for target $t$ against comparison set $C$ when
\[
  \forall h\in C,\qquad P\cap M(t,h)\neq\varnothing.
\]
Thus a marker panel is a hitting set over the pairwise marker sets $\{M(t,h):h\in C\}$.
\end{definition}

In words, a single marker gene must recur across all required pairwise contrasts, whereas a marker panel may use different genes to cover different contrasts. This distinction is important experimentally: a strict single-gene marker may not exist, even when a small panel distinguishes the target from every specified alternative.

This framework begins after a study has chosen a measurement, a partition, and a marker-calling rule. It does not assert that the partition is biologically complete or that the assay contains a stable marker for every distinction. It states what can be concluded from the pairwise marker evidence that the study produced.

\section{Marker Comparisons Define Cell Types and States}

Fix a study $s$ and a partition $p$ of the cells in that study. The partition may be a clustering, a set of annotated cell types, disease strata, tissue compartments, or any other grouping used for marker analysis. Let $K_{s,p}$ be the finite set of groups in the partition, and let $G$ be the set of genes. For a target group $t\in K_{s,p}$ and a comparison background $h\in K_{s,p}$, write
\[
  M_{s,p}(t,h)\subseteq G
\]
for the pairwise marker set: the genes called as markers for $t$ against $h$ by the study's chosen differential-expression or marker-calling rule. At the contrast level, this can be implemented by, for example, a mean-expression function $\mu$ and a threshold $\tau$:
\[
  g\in M_{s,p}(t,h)
  \quad\Longleftrightarrow\quad
...
This theorem is the marker-gene definition used in the paper. A marker gene is not a property of $(t,g)$ alone. It depends on $(s,p,t,C,g)$, where $C$ records the groups the target was compared against. The set $C$ can be broad, such as all observed atlas cell types; local, such as immune populations in blood; or task-specific, such as a set of biologically confusable neighbors. These are different marker definitions.

\begin{corollary}[Pairwise reporting is reusable]\label{cor:pairwise-reporting-reusable}
If a study reports $M_{s,p}(t,h)$ for each pairwise target--comparison contrast in a partition, then a later user can reconstruct $M_{s,p}(t;C)$ for any set $C$ of comparison backgrounds contained in that partition by taking an intersection.
\end{corollary}
...
The panel version is analogous. A marker-panel-supported cell type is a nonempty family of local instances $I$ together with a finite panel $P$ such that every local instance is covered:
\[
  \forall i=(s_i,p_i,t_i,C_i)\in I,\qquad
  \forall h\in C_i,\qquad
  P\cap M_{s_i,p_i}(t_i,h)\neq\varnothing.
\]
This makes the cell type definition operational. A cell type is not only a name; it is a target group, observed in specified studies and partitions, that is identifiable from specified comparison backgrounds by a marker gene or marker panel. The label can then be reused only to the extent that the underlying marker context is retained.
...
Once markers and cell types include their comparison sets, the main question is how they change when comparison backgrounds are added or removed.
...
A one-vs-background mean contrast is therefore a weighted summary of pairwise mean contrasts. The same reasoning applies to $k$-vs-background mean contrasts after replacing the target mean by a weighted mean over the $k$ target groups. The singleton comparison sets and the full comparison set are the two ends of $\mathcal{P}(H)$, but a pooled one-vs-all statistic is not the intersection defined at the full-set end. In standard cell-level tests, the background is implicitly weighted by the number of cells in each non-target group, so abundant groups can dominate the contrast. Equal-weight backgrounds reduce this abundance effect, but they are still averages. They do not record whether the gene distinguished the target from each individual comparison background.
...
\paragraph{Example.}
Suppose a gene has mean 10 in the target, mean 0 in an easy comparison background, and mean 9 in a hard comparison background. With threshold 5, it passes the easy pairwise contrast and fails the hard pairwise contrast. A one-vs-all average can still pass if the easy group dominates the background, or fail if the hard group dominates. The pairwise report preserves which distinction failed.

\subsection{Aggregation Across Studies}

Aggregation first requires a consistent identity for each target and comparison background. Stable database identifiers, or validated mappings to them, provide the equality relation needed to decide whether two studies refer to the same group. They are therefore necessary for computing pair coverage across papers. They are not sufficient for marker validity: assigning the same identifier to two reported groups does not show that either study tested the marker against the same alternatives. Stable identifiers make comparison evidence joinable; the observed pairwise comparisons determine what the joined evidence supports.
...
\section{Finding Effective Marker Panels}

The definition above says what a marker panel is: a hitting set over pairwise marker sets. Finding a useful panel is a second problem. Biologists usually want a panel that covers the target comparisons, avoids off-target groups, and remains small enough for practical use.

This separates our definition from existing marker-panel algorithms. Tools such as COMET and MAGNETO optimize compact panels that isolate a target cell population from a fixed background in a given single-cell dataset \citep{Delaney2019COMET,Tangherloni2022MarkerPanelOptimization,Tangherloni2023MAGNETO}. Our formalism specifies what comparison task the panel is valid for; optimization then chooses among valid or candidate panels for that task.
...
\begin{tabular}{p{0.32\linewidth}p{0.58\linewidth}}
\toprule
Practical desideratum & Selection criterion \\
\midrule
The panel should identify the target across the relevant alternatives. & Maximize sensitivity: the fraction of target-vs-comparison contrasts covered by at least one selected gene. \\
The panel should avoid marking unrelated groups. & Minimize off-target hits: the number or fraction of non-target contexts hit by the selected genes. \\
The panel should be experimentally usable. & Minimize panel size: the number of genes in the panel. \\
\bottomrule
\end{tabular}
\caption{Practical desiderata for selecting useful marker panels. These criteria optimize among candidate panels after the marker-panel definition has been fixed.}
\label{tab:marker-panel-selection-desiderata}
\end{table}

Let $C^{+}$ be the positive contexts that should be covered and let $C^{-}$ be the off-target contexts that should be avoided. For a panel $P$, define
\[
  \mathrm{coverage}(P)
  =
  \left|\{c\in C^{+}:P\cap M(c)\neq\varnothing\}\right|
\]
and
\[
  \mathrm{offtarget}(P)
  ...
  \left|\{c\in C^{-}:P\cap M(c)\neq\varnothing\}\right|.
\]
The useful-panel problem is therefore multi-objective: increase coverage, decrease off-target hits, and decrease panel size $|P|$. A panel is Pareto-optimal when no other candidate panel has at least as much coverage, no more off-target hits, and no more genes, with one strict improvement. A single selected panel can be obtained only after adding a priority rule or tie-breaker, for example first maximizing coverage, then minimizing off-target hits, then minimizing size.

The Lean formalization proves the monotonicity behind this tradeoff. Adding genes can only increase target coverage, but it can also only increase off-target hits. It also proves a diminishing-returns statement: adding the same gene to a larger panel covers no more new positive contexts than adding it to a smaller panel. Thus marker-panel construction is a finite hitting-set or set-cover optimization problem once the candidate genes, positive contexts, and off-target contexts are specified.

\begin{proposition}[Binary marker-panel information bound]\label{prop:marker-panel-information-bound}
If a panel $P$ separates all cell groups in a finite universe $K$, then
\[
  |K|\leq 2^{|P|}.
\]
\end{proposition}
...
\begin{proof}
A panel $P$ separates a set of cell groups when every distinct pair differs on at least one gene in $P$. Therefore the binary marker signatures on $P$ are injective, and there are only $2^{|P|}$ possible signatures.
\end{proof}
...
This bound is separate from the experiment-coverage bound above. The coverage bound asks whether the required comparisons were observed. The marker-panel bound asks whether the available genes provide enough binary signatures to distinguish the cell groups being compared.

\paragraph{Example.}
Three binary marker genes can encode at most eight distinct marker signatures. If an atlas needs to distinguish more than eight cell groups by binary marker calls, then either more markers, non-binary measurements, or additional experimental information is required.

\section{Dropping the Binary Marker Assumption}

The binary assumption in this note is an assumption about the reported marker call, not about gene expression or differential expression itself. In practice, a pairwise comparison often produces a real-valued statistic: an effect size, test statistic, posterior probability, stability score, or calibrated marker score. For a fixed target, write $q(h,g)$ for the score of gene $g$ against comparison background $h$. This score can include variance, uncertainty, or stability. The formal result only requires that the score be evaluated for each pairwise comparison before thresholding. A binary marker call is then obtained by a decision rule, for example by choosing a threshold $\tau$ and declaring
\[
...
The definition of a marker gene is a gene that distinguishes a target group from every group in a specified comparison set. The definition of a marker panel is the corresponding hitting set over pairwise marker contrasts. Finding an effective panel is then an optimization problem over coverage, off-target hits, and panel size. Together, these statements motivate a marker-supported definition of cell type: a cell type is a target group, observed in specified studies and partitions, that is identifiable from specified comparison backgrounds by a marker gene or marker panel. This is why pairwise marker context should be retained for each partition in a single-cell study. Pairwise marker calls can be recombined for any comparison set contained in the reported partition, but they cannot create contrasts against groups that were not observed. Across studies, stable identifiers make overlapping target--comparison evidence joinable, while pair coverage determines which broader marker sets the combined evidence supports. If future users need pooled statistical tests, the report should also retain sufficient summaries such as means, variances, and sample sizes. One-vs-all lists and flat cell type--gene tables generally cannot support either reuse case on their own.
