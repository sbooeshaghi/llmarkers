...
\begin{figure*}[ht!]
  ...
  \caption{\textbf{Cross-study marker unification.} Top: corpus summary for the human-curated benchmark, bioRxiv preprints, and HCA publications after filtering to analysis-ready marker records and profiles with at least three mapped marker gene IDs. Middle left: cross-paper pairs stratified by whether they share an exact cell type label and at least one marker gene. Middle center: exact label agreement among nearest neighbors in marker space. Middle right: marker overlap among nearest neighbors in label space. Bottom left: manual review of ambiguous cross-paper pairs, separating cases where shared or partially shared marker profiles support the same name, a context distinction, or remain unresolved. Bottom middle and right: reviewed examples where marker profiles expose different labels with identical genes, or same labels with different marker profiles.}
...
% Narrative spine: Show that corpus-scale extraction is operationally feasible rather than just conceptually possible.
...
% Narrative spine: Normalize genes so marker profiles can be compared across papers on a common identifier space.
...
% Narrative spine: Quantify the nomenclature problem directly by showing how many differently named profiles share substantial marker overlap.
...
% Narrative spine: Validate that the marker-based groupings are not arbitrary by contrasting within-cluster and between-cluster similarity.
...
% Narrative spine: Balance the positive clustering result by showing that even same-named cell types can diverge across study contexts.
