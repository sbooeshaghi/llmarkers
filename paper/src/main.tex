...
Marker genes are a common shorthand for identifying cell types. By saying gene X marks cell type Y, scientists communicate practical and testable hypotheses about cells. This binary format is useful but lossy. When used to report markers from quantitative assays such as single-cell RNA-seq, it discards the experimental and statistical evidence underlying the marker. To measure this information loss, we built \texttt{LLMarkers}, a benchmark of 1{,}560 cell type--gene pairs extracted from the text and figures of seven single-cell RNA-seq studies, each linked to the differential expression gene (DEG) table from which the marker was derived. We find that marker selection is not reducible to a rank threshold on DEGs. We then asked whether large language models (LLMs) could aid marker curation. LLMs could not recover study-specific markers through zero-shot generation or DEG selection, but they reliably extracted markers from the manuscript text that described them. We developed \texttt{mrkr}, a tool for automated marker extraction and verification, and applied it to 504 bioRxiv preprints, extracting 12{,}501 marker associations across 2{,}720 cell types. Clustering these extracted profiles recovered known immune cell hierarchies across 249 papers despite variable nomenclature. We release these markers through \texttt{LLMarkersDB}, an open-source database of literature-linked markers.
...
We therefore view a reported marker not as an isolated cell type--gene pair, but as evidence for a local cell type partition within a study. A cell type partition is a context-specific distinction among related populations of cells defined by one or more measurable parameters, such as genes, proteins, assays, or other observed attributes. We refer to this representation as a cell type partition (CTP). A CTP preserves the parent context, the populations being distinguished, the parameters used to distinguish them, and the textual or quantitative evidence supporting those distinctions.

Large language models offer a new way to find cell-type markers. With them, scientists can extract markers from manuscripts, generate them from training knowledge alone, or annotate cell types from differential expression gene lists. \citep{Hou_2024} demonstrated the latter, showing that LLMs have internalized marker--cell type relationships from their training data. However, by solely relying on the DEG table to annotate cell types, LLMs ignore the manuscript and the specific markers that the authors---the domain experts---chose to report. A recent large-scale benchmark found that LLMs ``profoundly outperform traditional methods'' of cell type annotation across 34 datasets, but relied on a custom scoring pipeline, that heavily relied on Cell Ontology terms, which may overinflate accuracy for cell types whose nomenclature maps cleanly onto ontology terms while penalizing novel subtypes that lack ontology terms \citep{Xiao_2025}. 

% That enable us to evaluate marker genes
We propose an alternative approach to curating marker genes. LLMs can be used to extract markers directly from a manuscript that reports them. In extraction mode, the model reads a manuscript and pulls out markers that the authors reported, along with the sentences that support them. These markers are tied to a specific paper, can be verified against it, and preserve the evidence needed to construct CTPs. In contrast, in generation mode, the model produces markers from its training knowledge alone, without a manuscript, DEG table, or experimental context. The LLM infers connections between cell types and genes from its training corpus, but does not directly tie them to any specific experiment.

In this study, we investigate marker genes reported in scientific texts and evaluate how well LLMs can recover them while preserving the evidence needed to represent cell type partitions. We build a benchmark of 1{,}560 cell type--gene pairs from seven single-cell RNA-seq studies, each linked to the specific DEG table from which the marker was derived. Using this benchmark, we show that authors tend to report strong markers, but that marker selection is not reducible to a simple rank threshold on differential expression. We compare LLM-based marker generation, selection, and extraction against human curation and develop \texttt{mrkr}, a tool that automates marker extraction from manuscripts. Our results show that text-based extraction recovers paper-specific markers with high precision, whereas generated and selected markers---though biologically plausible---are drawn from a broader range of differentially expressed genes. Finally, we show that marker profiles extracted from a large corpus of bioRxiv preprints recover recurring cell-type relationships despite variable nomenclature, providing the building blocks for representing cell type partitions at corpus scale.
