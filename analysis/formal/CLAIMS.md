...
4. **Information bound.** If a marker panel `S` separates `k` cell types, then
   `k <= 2 ^ |S|`. Equivalently, any binary identifying panel requires at least
   `ceil(log2 k)` markers in the unconstrained best case.

5. **Monotonicity of separation.** If `S` separates the cell types, then every
   superset of `S` also separates them.

6. **Hierarchy/refinement.** Adding markers refines the induced partition. If
   two profiles agree on a larger marker panel, they also agree on every smaller
   panel.

7. **Scope poset.** For a fixed comparison set, the separating marker panels are
...
8. **Non-uniqueness.** Minimum separating marker panels need not be unique. The
   formal example proves that two different one-gene panels can both be minimum
   separating panels.

9. **Gene classes.** The file defines essential, exchangeable, and redundant
   genes relative to the family of minimum separating panels.

10. **Local/global separation.** Separation over a global comparison set implies
...
11. **Adding a comparison cell type can change marker status.** A one-gene panel
...
12. **Marker liftover.** The file defines separation of one profile from a
...
13. **Experiment pair coverage.** A global marker panel can be certified from
   ...
   If there are `K` target cell types and each experiment jointly compares at
   most `r` cell types, then any pair-covering design needs at least
   `ceil(choose(K, 2) / choose(r, 2))` experiments. The exact optimum is the
   covering design number `C(K, r, 2)` and can be larger.

14. **Admissible comparison graphs.** If only some cell-type pairs are
...
15. **Ideal one-vs-all equivalence.** At the binary marker-signature level,
