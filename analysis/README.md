...
Run Python scripts from the repository root so paths such as `analysis/results/` and `docs/paper/biorxiv/src/` resolve as written. The notebooks were authored inside this directory and use notebook-local paths such as `figures/...`; run them with `analysis/` as the working directory if regenerating notebook-only artifacts.
